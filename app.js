// AstaMasterPro v9c2 — app.js [patch v1.2.1: rank FM/MV per anno separati + toggle badge prev]
(function () {
  'use strict';
  ...
  function setupTheme(){ var saved = localStorage.getItem(THEME_KEY); applyTheme(((saved==='light'||saved==='dark')?saved:'dark')); var sw=$('#switchTheme'); if(sw) sw.addEventListener('change',function(e){ var mode=e.target.checked?'dark':'light'; applyTheme(mode); localStorage.setItem(THEME_KEY, mode); }); }
  ...
  var OPT_KEYS = {
    ...
    showPrevBadges: 'amp-showPrevBadges', // toggle badge stag. precedente
    ...
    // v1.2.1 — rank FM/MV per stagione
    rankBadgeSeasonFM: 'amp-rankBadgeSeasonFM',
    rankBadgeSeasonMV: 'amp-rankBadgeSeasonMV',
    // v1.2 — badge valori/aff per stagione
    badgeSeasonValues: 'amp-badgeSeasonValues',
    badgeSeasonAff: 'amp-badgeSeasonAff'
  };
  var OPTS = null;
  ...
  function loadOptions(){
    ...
    var o = {
      ...
      showPrevBadges: qb(OPT_KEYS.showPrevBadges,false),
      ...
      // v1.2.1
      rankBadgeSeasonFM: qs(OPT_KEYS.rankBadgeSeasonFM,""),
      rankBadgeSeasonMV: qs(OPT_KEYS.rankBadgeSeasonMV,""),
      // v1.2
      badgeSeasonValues: qs(OPT_KEYS.badgeSeasonValues,""),
      badgeSeasonAff: qs(OPT_KEYS.badgeSeasonAff,"")
    };

    // compat 1.2: se esisteva la select unica, riusa per FM
    var legacy = localStorage.getItem('amp-rankBadgeSeason');
    if (legacy && !o.rankBadgeSeasonFM && !o.rankBadgeSeasonMV) o.rankBadgeSeasonFM = legacy;
    ...
    set('opt_rankBadgeSeasonFM', o.rankBadgeSeasonFM);
    set('opt_rankBadgeSeasonMV', o.rankBadgeSeasonMV);
  ...
  function persistOptions(){
    ...
    localStorage.setItem(OPT_KEYS.rankBadgeSeasonFM, OPTS.rankBadgeSeasonFM||'');
    localStorage.setItem(OPT_KEYS.rankBadgeSeasonMV, OPTS.rankBadgeSeasonMV||'');
  ...
  function populateHistSeasonOptions(){
    ...
    if(!OPTS.histSelSeasons || !OPTS.histSelSeasons.length){
      OPTS.histSelSeasons = SEASONS.map(function(s){ return s.seasonKey; }); persistOptions();
    }
  ...
  function populateAllBadgeSeasonOptions(){
    var map = [
      { id:'opt_rankBadgeSeasonFM', key:'rankBadgeSeasonFM' },
      { id:'opt_rankBadgeSeasonMV', key:'rankBadgeSeasonMV' },
    ...
    map.forEach(function(m){
      ...
      SEASONS.forEach(function(s){ var o=document.createElement('option'); o.value=s.seasonKey; o.textContent=seasonStr(s.seasonKey); el.appendChild(o); });
      var sel = OPTS[m.key] || ''; if(sel && !SEASONS.some(function(x){return x.seasonKey===sel;})) sel='';
  ...
  function setupOptions(){
    ...
    ['opt_showPrevBadges','opt_showDelta','opt_showDetails','opt_histShowBM','opt_histShowPerPres','opt_useIndicators','opt_showRolePos']
    ...
    function bindSel(id,key){ var el=document.querySelector('#'+id); if(!el) return;
      el.addEventListener('change', function(){ OPTS[key]=el.value||''; persistOptions(); if(SELECTED_COD) selectPlayer(SELECTED_COD); });
    }
    bindSel('opt_rankBadgeSeasonFM','rankBadgeSeasonFM');
    bindSel('opt_rankBadgeSeasonMV','rankBadgeSeasonMV');
  ...
  async function discoverSeasons(){
    ...
    if(!out.length) console.error('Nessuna stagione trovata. Assicurati che i file JSON siano in /data e si chiamino YYYY_YYYY+1.json o YYYY.json o YYYY-YYYY+1.json');
  ...
  function buildIndex(){
    ...
    CUR.players.forEach(function(c){
      ...
      INDEX.push({ cod:String(c.cod), nome:c.nome, ruolo:c.r, squadra:c.sq,
  ...
  function renderResults(items, q){
    ...
    items.slice(0,120).forEach(function(item){
      var li=document.createElement('li'); li.className='result-item'; li.setAttribute('role','option'); li.tabIndex=0;
  ...
  function search(q){
    var nq=norm(q), ul=$('#results'); if(!nq){ if(ul) ul.innerHTML=''; return; }
  ...
  function computeRankForSeason(seasonObj, metric, value, role, minPres){
    try{
      ...
      var rank=Math.max(1, N-(less+equal)+1); // ex-aequo
      return { pct:pct, z:z, N:N, rank:rank };
    }catch(e){ return null; }
  }

  function addBadge(arr, text, cls, title){ if(cls===void 0) cls='info';
  ...
  function renderCurrent(rec){
    var el=$('#cardCurrent'); if(!rec){ el.innerHTML='<div class="small">Giocatore non trovato nella stagione corrente.</div>'; return; }
    ...
    if(OPTS.showRolePos){
      ...
      if(rkFM || rkMV){
        ...
        if(rkFM){ lines.push('<div class="row"><span class="key">FM '+escapeHtml(y)+' – Percentile ('+rkFM.rank+'° su '+rkFM.N+')</span><span class="val">'+fmt(rkFM.pct*100,1).replace('.', ',')+'%</span></div>'); lines.push(row('FM – Z‑score', rkFM.z)); }
        if(rkMV){ lines.push('<div class="row"><span class="key">MV '+escapeHtml(y)+' – Percentile ('+rkMV.rank+'° su '+rkMV.N+')</span><span class="val">'+fmt(rkMV.pct*100,1).replace('.', ',')+'%</span></div>'); lines.push(row('MV – Z‑score', rkMV.z)); }
  ...
  function renderPrev(rec, prevSeasonKey){
    var el=$('#cardPrev'); if(!rec){ el.innerHTML='<div class="small">Nessun dato stagione precedente.</div>'; return; }
    var isP=(rec.r==='P'), parts=[];
    parts.push(row('Squadra', rec.sq));
    parts.push(row('Ruolo', rec.r));
    parts.push(rowInt('Presenze', rec.p));
  ...
  function renderStorico(cod){
    ...
    SEASONS.forEach(function(s){ var rec=s.players.find(function(p){ return String(p.cod)===String(cod); }); if(rec) found.push({seasonStart:s.seasonStart, seasonKey:s.seasonKey, rec:rec}); });
    ...
    var filtered=found.filter(function(x){ return sel.has(x.seasonKey); }); if(!filtered.length){ el.innerHTML='<div class="small">Nessun dato storico per le stagioni selezionate.</div>'; return; }
    filtered.sort(function(a,b){ return a.seasonStart - b.seasonStart; });

    var n=filtered.length, list=filtered.map(function(x){return seasonStr(x.seasonKey);}).join(', ');
    ...
    var mvW= presTot ? (mvNum/presTot) : null, fmW= presTot ? (fmNum/presTot) : null;
    ...
    if(!isP){ dati.push(rowInt('Gol totali', gfTot)); dati.push(rowInt('Gol su rigore totali', gfrTot)); dati.push(rowInt('Rigori sbagliati totali', rsTot)); }
    else { dati.push(rowInt('GS totali', gsTot)); dati.push(rowInt('GSR totali', gsrTot)); dati.push(rowInt('RP totali', rpTot)); }
    ...
    if(OPTS.histShowBM){
      var bm=[];
      if(!isP){ bm.push(row('Gol medi/stagione', n?(gfTot/n):null)); bm.push(row('Gol su rigore medi/stagione', n?(gfrTot/n):null)); bm.push(row('Rigori sbagliati medi/stagione', n?(rsTot/n):null)); }
      else { bm.push(row('GS medi/stagione', n?(gsTot/n):null)); bm.push(row('GSR medi/stagione', n?(gsrTot/n):null)); bm.push(row('RP medi/stagione', n?(rpTot/n):null)); }
    ...
    if(OPTS.histShowPerPres){
      var sp=[];
      if(!isP){ sp.push(row('Presenze per gol', gfTot>0? presTot/gfTot : null));
      ...
      else    { sp.push(row('GS per presenza', (presTot>0&&gsTot!=null)?(gsTot/presTot):null));
  ...
  // --------- BADGE: Rank per stagione (FM, MV o entrambi) ---------
  function buildSeasonRankingBadges(seasonObj, seasonRec, which /* 'FM'|'MV'|'both' */){
  ...
  function selectPlayer(cod){
    ...
    var role = (idx && idx.ruolo ? idx.ruolo : (curRec ? curRec.r : '')); role=String(role||'').toUpperCase();
    ...
    var hdr=[];
    // (A) Segnali prev (cambio ruolo/squadra + badge prev su soglie) — solo se abilitato
    if(OPTS.showPrevBadges){
      ...
      if(PREV && prevRec){
        var yPrev=seasonStr(PREV.seasonKey);
        if(Number(prevRec.mvt) >= OPTS.mvPrevBadge) addBadge(hdr, 'MV '+yPrev+' '+fmt(prevRec.mvt,2), 'good', 'Prev • Soglia: '+OPTS.mvPrevBadge);
        if(Number(prevRec.fmt) >= OPTS.fmPrevBadge) addBadge(hdr, 'FM '+yPrev+' '+fmt(prevRec.fmt,2), 'good', 'Prev • Soglia: '+OPTS.fmPrevBadge);
        var affp=prevRec.aff; var affPrevPct=(affp!=null ? (Number(affp)>1?Number(affp):Number(affp)*100) : null);
        if(affPrevPct!=null && affPrevPct>=OPTS.affPrevBadge) addBadge(hdr,'Aff '+yPrev+' '+fmtPercent(prevRec.aff),'good','Prev • Soglia: '+OPTS.affPrevBadge+'%');
      }
    }

    // (B) Rank FM/MV — stagione scelta (prima)
    if(OPTS.rankBadgeSeasonFM){
      var sFM = SEASONS.find(function(s){ return s.seasonKey===OPTS.rankBadgeSeasonFM; });
      var rFM = sFM && sFM.players.find(function(p){ return String(p.cod)===String(cod); });
      if(rFM) buildSeasonRankingBadges(sFM, rFM, 'FM').forEach(function(x){ hdr.push(x); });
    }
    if(OPTS.rankBadgeSeasonMV){
      var sMV = SEASONS.find(function(s){ return s.seasonKey===OPTS.rankBadgeSeasonMV; });
      var rMV = sMV && sMV.players.find(function(p){ return String(p.cod)===String(cod); });
      if(rMV) buildSeasonRankingBadges(sMV, rMV, 'MV').forEach(function(x){ hdr.push(x); });
    }

    // (C) FM/MV — stagione scelta (valori con soglie)
