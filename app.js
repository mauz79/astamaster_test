...
// Base: v1.2 (selettori anno per badge) — mantenute tutte le funzionalità esistenti
(function () {
  'use strict';
  ...
  function applyTheme(mode) {
    ...
    var sw = $('#switchTheme'); if (sw) sw.checked = (mode === 'dark');
  }
  function setupTheme() {
  ...
  var OPT_KEYS = {
    ...
    showPrevBadges: 'amp-showPrevBadges',        // NEW: toggle badge prev season
    ...
    // v1.2.1: split FM/MV
    rankBadgeSeasonFM: 'amp-rankBadgeSeasonFM',  // Rank FM — stagione scelta
    rankBadgeSeasonMV: 'amp-rankBadgeSeasonMV',  // Rank MV — stagione scelta
    // v1.2: badge valori su stagione scelta
  ...
  function loadOptions() {
    ...
    // Migrazione leggera: se provengo dalla 1.2 e avevo la select unica, usala come default per FM.
  ...
  // Popola i select legati alle stagioni (rank FM/MV, valori FM/MV, Aff)
  function populateAllBadgeSeasonOptions(){
    ...
    map.forEach(function (m){
      ...
      var sel = OPTS[m.key] || ''/*non forziamo default*/;
  ...
  function setupOptions() {
    ...
    function syncBools(){
      OPTS.showPrevBadges = $('#opt_showPrevBadges').checked;
  ...
  // v1.2.1 — Badge Ranking per stagione scelta (singolo o entrambi)
  ...
  function selectPlayer(cod) {
    ...
    // 1) Rank FM — stagione scelta (PRIMA)
    if (OPTS.rankBadgeSeasonFM) {
      var s1fm = SEASONS.find(function (s) { return s.seasonKey === OPTS.rankBadgeSeasonFM; });
      var r1fm = s1fm && s1fm.players.find(function (p) { return String(p.cod) === String(cod); });
      if (r1fm) buildSeasonRankingBadges(s1fm, r1fm, 'FM').forEach(function (x) { hdr.push(x); });
    }
    // 1b) Rank MV — stagione scelta
    if (OPTS.rankBadgeSeasonMV) {
      var s1mv = SEASONS.find(function (s) { return s.seasonKey === OPTS.rankBadgeSeasonMV; });
      var r1mv = s1mv && s1mv.players.find(function (p) { return String(p.cod) === String(cod); });
      if (r1mv) buildSeasonRankingBadges(s1mv, r1mv, 'MV').forEach(function (x) { hdr.push(x); });
    }
    ...
    // 4) (NUOVO) Badge stagione precedente — opzionali via toggle
    if (OPTS.showPrevBadges && PREV && prevRec) {
  ...
  function wireSearch() {
    ...
    input.addEventListener('input', function (e) {
      ...
      if (v && v.length > 0) showResults(); else hideResults();
      clearTimeout(wireSearch._t);
      wireSearch._t = setTimeout(function () { search(v); }, 150);
    });
  }
  document.addEventListener('keydown', function (e) {
    if ((e.ctrlKey || e.metaKey) && (e.key === 'q' || e.key === 'Q')) {
      var el = $('#searchInput');
      if (el) { e.preventDefault(); el.focus(); if (el.select) el.select(); showResults(); }
    }
  });
